...
/**
 * Check whether givven node is `ReturnStatement` of a React hook
 * @param {ASTNode} node The AST node
...
function isComponentInRenderProp(node) {
  ...
  // Check whether component is created by Array.map call, e.g. <Component>{items.map(() => <div />)}</Component>
  // These should be valid, mark them as render prop
  if (
    node
    && node.parent
  ...
  return (
    ...
    && RENDER_REGEXP.test(jsxExpressionContainer.parent.name.name)
  );
}
...
module.exports = {
  ...
  create: Components.detect((context, components, utils) => {
    ...
    function isComponentInProp(node) {
      ...
      // Check if we are in Array.map call, e.g. <List items={data.map(() => <li />)} />
      if (
        jsxAttribute
        && jsxAttribute.value
        && jsxAttribute.value.expression
        && jsxAttribute.value.expression.callee
        && jsxAttribute.value.expression.callee.property
        && jsxAttribute.value.expression.callee.property.name === 'map'
      ) {
        return false;
      }
